 ...
 * @since 2018
 */
public class IntFieldCodec implements FieldCodec {
    private final Getter get;
    private final Setter set;
    private final Codec codec;

    public IntFieldCodec(
            Method getter,
            Method setter,
            Codec codec) throws CodecGenerationException {
        super();
        get = LambdaMetaFactoryUtils.wrapGetter(Getter.class, getter, int.class);
        set = LambdaMetaFactoryUtils.wrapSetter(Setter.class, setter, int.class);
        this.codec = codec;
    }
    ...
    public void read(InputStreamWrapper wrapper, Object instance) throws IOException {
        set.set(instance, (Integer) codec.read(wrapper));
    }
