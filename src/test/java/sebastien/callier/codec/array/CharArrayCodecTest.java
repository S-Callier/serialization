...
public class CharArrayCodecTest {
    ...
    private static final CharArrayCodec codec = new CharArrayCodec((byte) 0);
    private static final CharArrayCodec invalidCodec = new CharArrayCodec((byte) 50);
    ...
    public void testStringWriteRead() throws IOException {
        ...
        for (int i = 1; i < 200; i++) {
            char[] b = new char[i];
            for (int j = 0; j < i; j++) {
                //lazily avoid invalid UTF_8
                b[j] = (char) random.nextInt(256);
            }
            testCharArray(b);
        }
    }
    ...
    public void testInvalidData() throws IOException {
        ...
        serializer.append(new char[0], invalidCodec);
        serializer.close();
    ...
    private void testCharArray(char[] value) throws IOException {
        Serializer serializer = serializerFactory.newSerializer();
