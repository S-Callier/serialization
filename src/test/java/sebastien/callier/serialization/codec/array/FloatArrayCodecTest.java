...
public class FloatArrayCodecTest {
    ...
    private static final FloatArrayCodec codec = new FloatArrayCodec((byte) 0);
    private static final FloatArrayCodec invalidCodec = new FloatArrayCodec((byte) 50);
    ...
    @Test
    public void floatArray() throws IOException {
        testFloatArray(null);
        testFloatArray(new float[]{});
        for (int i = 1; i < 200; i++) {
            float[] b = new float[i];
            for (int j = 0; j < i; j++) {
    ...
    public void testInvalidData() throws IOException {
        ...
        serializer.append(new float[0], invalidCodec);
        serializer.close();
    ...
    private void testFloatArray(float[] value) throws IOException {
        Serializer serializer = serializerFactory.newSerializer();
