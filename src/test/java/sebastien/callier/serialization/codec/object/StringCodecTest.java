...
public class StringCodecTest {
    private static final StringCodec codec = new StringCodec((byte) 0, 100);
    private static final StringCodec invalidCodec = new StringCodec((byte) 50, 100);
    ...
    public void testStringWriteRead() throws IOException {
        testString(null);
        testString("");
        StringBuilder builder = new StringBuilder("abcdf日本語");
        do {
            testString(builder.toString());
            builder.append(builder.toString());
        } while (builder.toString().getBytes(StandardCharsets.UTF_8).length < 8388608 << 2);
    }

    @Test
    public void testInvalidData() throws IOException {
        Serializer serializer = serializerFactory.newSerializer();
        serializer.append("nope", invalidCodec);
        serializer.close();
    ...
    private void testString(String value) throws IOException {
        Serializer serializer = serializerFactory.newSerializer();
