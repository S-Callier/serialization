...
public class ByteCodecTest {
    private static final ByteCodec codec = new ByteCodec((byte) 0);
    private static final ByteCodec invalidCodec = new ByteCodec((byte) 50);
    ...
    public void testWideRange() throws IOException {
        testByte(null);
        for (byte value = 1; value > 0; value *= 2) {
            testByte((byte) (value - 1));
            testByte(value);
            testByte((byte) (value + 1));
        }
        for (byte value = -1; value < 0; value *= 2) {
    ...
    public void testInvalidData() throws IOException {
        ...
        serializer.append((byte) 12, invalidCodec);
        serializer.close();
    ...
    private void testByte(Byte value) throws IOException {
        Serializer serializer = serializerFactory.newSerializer();
